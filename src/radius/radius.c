...
static void radius_msg_dump_attr(struct radius_attr_hdr *hdr)
{
	...
	if (ext) {
		...
		WIFIMON_INF(NULL, S_UNK, WMC_OK, 0, 0, "attr=%d.%d name=%s length=%d",ext->type, ext->ext_type, attr ? attr->name : "?Unknown?", ext->length);
		...
		WIFIMON_INF(NULL, S_UNK, WMC_OK, 0, 0, "attr=%d name=%s length=%d", hdr->type, attr ? attr->name : "?Unknown?", hdr->length);
	...
	case RADIUS_ATTR_TEXT:
		...
		WIFIMON_INF(NULL, S_UNK, WMC_OK, 0, 0, "val=%s", buf);
	...
	case RADIUS_ATTR_IP:
		if (len == 4) {
			...
			WIFIMON_INF(NULL, S_UNK, WMC_OK, 0, 0, "val=%s", inet_ntoa(addr));
			wpa_printf(MSG_INFO, "      Value: %s", inet_ntoa(addr));
		} else {
			WIFIMON_INF(NULL, S_UNK, WMC_OK, 0, 0, "invalid_ip_len=%d", len);
	...
	case RADIUS_ATTR_IPV6:
		if (len == 16) {
			...
			WIFIMON_INF(NULL, S_UNK, WMC_OK, 0, 0, "val=%s", atxt ? atxt : "?");
			wpa_printf(MSG_INFO, "      Value: %s", atxt ? atxt : "?");
		} else {
			WIFIMON_INF(NULL, S_UNK, WMC_OK, 0, 0, "invalid_ipv6_len=%d", len);
	...
	case RADIUS_ATTR_INT32:
		if (len == 4){
			WIFIMON_INF(NULL, S_UNK, WMC_OK, 0, 0, "val=%u", WPA_GET_BE32(pos));
			wpa_printf(MSG_INFO, "      Value: %u", WPA_GET_BE32(pos));
		} else {
			WIFIMON_INF(NULL, S_UNK, WMC_OK, 0, 0, "invalid_int32_len=%d", len);
...
void radius_msg_dump(struct radius_msg *msg)
{
	...
	WIFIMON_INF(NULL, S_UNK, WMC_OK, 0, 0, "RADIUS_message code=%d name=%s identifier=%d length=%d",
			       ...
			       size_t secret_len, const u8 *req_auth)
{
	...
	if (hmac_md5(secret, secret_len, wpabuf_head(msg->buf),
		     wpabuf_len(msg->buf), auth) < 0) {
		wpa_printf(MSG_INFO, "RADIUS: MD5 not available");
		WIFIMON_ERR(NULL, S_UNK, WMC_LOC_UNK, 0, 0,
